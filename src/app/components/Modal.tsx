...
export default function Modal(props: Props) {
  ...
  const handleCreate = async () => {
    if (validateItem(selectedItem)) {
      const close = await crudsCollections[collectionName]?.createItem(
        selectedItem
      );
  ...
  const handleUpdate = async () => {
    if (validateItem(selectedItem)) {
      const close = await crudsCollections[collectionName].updateItem(
        selectedItem
      );
      if (close) setShowModal(false);
      window.location.reload();
    ...
    <div style={{margin: "4vh 0"}}>
      {showModal && (
        <div
          ...
          style={{ background: "#FDFDFD", boxShadow: "5px 5px 5px rgb(0,0,0, 0.3)" }}
        >
          <DetailsSelector
            func={labelButton}
